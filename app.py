...
st.set_page_config(page_title="Child-Safe AI Mentor", page_icon="🤖", layout="centered")
page = st.sidebar.selectbox("Select Page", ["Chat with Mentor✨", "Parent Review Panel📙"])

if "auto_flagged" not in st.session_state:
    st.session_state.auto_flagged = False  # Initialize auto-flagged session state

if page == "Chat with Mentor✨":
    st.markdown("""
        <h1 style='text-align: center;'>🤖 Child-Safe AI Mentor</h1>
    ...
    user_input = st.text_area("💬 What would you like to ask your mentor?", height=120)
    ...
    if st.button("🧠 Ask Mentor"):
        st.session_state.auto_flagged = False  # Reset the auto-flagged flag on a new question
            ...
            with st.spinner("Thinking positive thoughts... 🤔✨"):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Save to session state for flagging
                        ...
                        # Automatically flag the response if the error message matches
                        if error_msg == "That question may not be safe to ask." or error_msg == "Sorry, the response could not be shown.":
                            ...
                            st.session_state.auto_flagged = True  # mark as auto-flagged
                except requests.exceptions.RequestException:
                    st.error("⚠️ Failed to connect to the mentor API. Please try again later.")

    # 🚩 Flag this response (only if not auto-flagged)
    ...
    log_file = "flagged_responses.log"
            ...
            for i, entry in enumerate(logs):
                with st.expander(f"🚩 Flag #{i + 1}", expanded=False):
                    ...
                    if st.button(f"🗑️ Delete This Flag", key=f"delete_flag_{i}"):
                        ...
                        # Only write back flags that have actual content
                        with open(log_file, "w", encoding="utf-8") as f:
                            for flag in logs:
                                cleaned_flag = flag.strip()
                                if cleaned_flag:  # Skip writing empty entries
                                    f.write(cleaned_flag + "\n---END-FLAG---\n")

                        st.success(f"✅ Flag #{i + 1} deleted.")
                        st.rerun()
